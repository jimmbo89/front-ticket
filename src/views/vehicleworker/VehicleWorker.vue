<template>
  <div class="vehicle-workers-page">
    ...
    <header class="page-header">
      <div class="page-heading">
        <div class="vehicle-image-shell">
          <v-img v-if="vehicle.image" :src="vehicleImage" width="38" height="38" cover>
            <template #error><div class="vehicle-image-fallback"><v-icon size="20">mdi-bus</v-icon></div></template>
          </v-img>
          <v-icon v-else size="20">mdi-bus</v-icon>
        </div>
        <div>
          <h1 class="page-title">Trabajadores del vehículo</h1>
          <p class="page-subtitle"><strong>{{ vehicle.plate || "Sin patente" }}</strong><span v-if="vehicle.internal_number"> #{{ vehicle.internal_number }}</span></p>
        </div>
      </div>
    ...
    <v-container fluid class="page-content">
      <v-row class="summary-row">
        <v-col cols="12" sm="6"><div class="summary-card"><div class="summary-icon summary-icon--blue"><v-icon size="19">mdi-account-group-outline</v-icon></div><div><div class="summary-value">{{ vehicleworkers.length }}</div><div class="summary-label">Trabajadores asociados</div></div></div></v-col>
        <v-col cols="12" sm="6"><div class="summary-card"><div class="summary-icon summary-icon--green"><v-icon size="19">mdi-bus</v-icon></div><div><div class="summary-value">{{ vehicle.plate || "—" }} #{{ vehicle.internal_number }}</div><div class="summary-label">Vehículo seleccionado</div></div></div></v-col>
      </v-row>
          ...
          <template #[`item.name`]="{ item }">
            <div class="worker-name-cell">
              <div class="worker-avatar">
                <v-img v-if="item.image" :src="workerImage(item.image)" class="worker-photo" width="34" height="34" cover>
              ...
              <div class="cell-copy"><div class="worker-name">{{ item.name || "Sin nombre" }}</div><div class="worker-caption">Trabajador asignado</div></div>
            </div>
          </template>
          <template #[`item.email`]="{ item }"><span class="worker-email">{{ item.email || "Sin correo" }}</span></template>
        ...
        <v-card class="form-dialog" elevation="0">
          <div class="dialog-header">
            <div class="dialog-heading">
              ...
              <div><div class="dialog-title">{{ formTitle }}</div><div class="dialog-subtitle">Asigna varios trabajadores al vehículo {{ vehicle.plate || "seleccionado" }} en una sola operación</div></div>
            </div>
    ...
    <v-dialog v-model="dialogDelete" max-width="430" persistent>
      <v-card class="delete-dialog" elevation="0">
        ...
        <div class="delete-message">¿Deseas quitar a <strong>{{ editedItem.name || "este trabajador" }}</strong> del vehículo? El trabajador no será eliminado del sistema.</div>
...
export default {
  ...
  emits: ["close"],
  props: {
    vehicle: { type: Object, required: true, default: () => ({}) },
  },
  data: () => ({
    snackbar: false,
    sb_type: "",
    ...
    defaultItem: { id: "", vehicle_id: "", worker_id: [], name: "" },
    editedIndex: -1,
    search: "",
  ...
  computed: {
    ...
    registeredCountText() {
      const count = this.vehicleworkers.length;
      return `${count} ${count === 1 ? "trabajador asociado" : "trabajadores asociados"}`;
    },
    vehicleImage() {
      return `${this.$axios.defaults.baseURL}images/${this.vehicle.image}?t=${this.getCacheTimestamp()}`;
    },
    selectedWorkerCount() {
  ...
  methods: {
      ...
      try {
        ...
        if (result.success) {
          this.workers = result.data?.workers.filter((worker) =>
            !this.vehicleworkers.some((vehicleworker) => String(vehicleworker.worker_id) === String(worker.id))
          ) || [];
        } else {
          this.workers = [];
        }
    ...
    async initialize() {
      try {
        ...
        const result = await handleRequest({ endpoint: "vehicle-workers", method: "POST", data: this.data });
        this.vehicleworkers = result.success ? result.data?.vehicleWorkers || [] : [];
      } catch (error) {
        this.vehicleworkers = [];
        this.showAlert("error", "Ocurrió un error inesperado al procesar la solicitud.", 3000);
      } finally {
    ...
    async save() {
      ...
      if (this.editedIndex === -1) {
        ...
        for (const workerId of workerIds) {
          try {
            const result = await handleRequest({
              ...
              method: "POST",
              data: { vehicle_id: this.vehicle_id, worker_id: workerId },
            });
        ...
        if (Object.keys(updatedFields).length > 0) {
          ...
          try {
            const result = await handleRequest({ endpoint: "vehicle-worker", method: "PUT", data: updatedFields });
            if (result.success) {
    ...
    async editItem(item) {
      ...
      try {
        ...
        if (result.success) {
          this.workers = result.data?.workers.filter((worker) =>
            !this.vehicleworkers.some((vehicleworker) => vehicleworker.worker_id === worker.id) || worker.id === this.editedItem.worker_id
          ) || [];
        }
      } catch (error) {
    ...
    async deleteItemConfirm() {
      ...
      try {
        const result = await handleRequest({ endpoint: "vehicle-worker-destroy", method: "POST", data: { id: this.editedItem.id } });
        if (result.success) {
          this.showAlert("success", result.message, 3000);
          await this.initialize();
        } else {
...
<style scoped>
.vehicle-workers-page { --blue:#2454d6; min-height:100%; color:#1e293b; background:#f6f8fb; }
...
.page-heading,.dialog-heading { display:flex; align-items:center; gap:11px; }.vehicle-image-shell,.dialog-icon { position:relative; display:grid; flex:0 0 38px; width:38px; height:38px; overflow:hidden; place-items:center; color:#fff; background:radial-gradient(circle at 90% 5%,rgba(53,184,232,.5),transparent 28px),linear-gradient(135deg,#0e1f46,#2454d6); border-radius:10px; box-shadow:0 5px 12px rgba(36,84,214,.17); }.vehicle-image-fallback { display:grid; width:100%; height:100%; place-items:center; }.dialog-icon-main { transform:translate(-2px,1px); }.dialog-icon-action { position:absolute; right:5px; bottom:5px; padding:1px; color:#0e1f46; background:#fff; border-radius:50%; }
